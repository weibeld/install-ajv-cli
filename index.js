...
async function main() {
  try {
    ...
    if (!version) {
      ...
      fail(`${version} is not a valid version of ajv-cli (list existing versions with 'npm view ajv-cli versions')`);
    }

    key = `ajv-cli-${version}_node-${process.version}_${process.env.RUNNER_OS}`;
